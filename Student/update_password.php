...
if($_POST['cur_pas'] && $_POST['new_pas1'] && $_POST['new_pas2']){
    ...
    while($row = mysqli_fetch_assoc($res_stu)){
        if($row['student_email']==$_SESSION['email'] && $row['student_password']==$_POST['cur_pas']){
            $s_id = $row['student_id'];
            break;
        }
    }
    ...
    if($s_id!=0){
        $q_update = "UPDATE STUDENT SET student_password='$pass' WHERE student_id = '$s_id' ";
